...
void TabouSearch::initTables(){
  ...
  for (int i=0; i<nbSommets; i++) {
    ...
    for (int j = graph->tConnect[i]._Find_first(); j < nbSommets; j = graph->tConnect[i]._Find_next(j)) {
      tNewConflitsWithColor[i][currentSol.tColor[j]]++;
    }
...
void TabouSearch::updateTables(int node, int color){
  ...
  //for (int i=graph->tConnect[node]._Find_first(); i<graph->nbSommets; i = graph->tConnect[node]._Find_next(i)) {
    //int indiceSommet = i;
