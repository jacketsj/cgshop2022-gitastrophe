...
void Head::compute(){
  ...
  if (startingConf!=""){ /// case where we start from a previous configuration
    ...
    if (initSol) {
      vPopulation[0] = *initSol;//Solution(graph, nbColors);
      vPopulation[1] = *initSol2;//Solution(graph, nbColors);
      vPopulation[2] = *initSol;//Solution(graph, nbColors); ///  individu elite1
      vPopulation[3] = *initSol2;//Solution(graph, nbColors); ///  individu elite2
    } else {
...
#pragma omp parallel for
      ...
      if (swapIter>0 && (nbIterationsCross%swapIter==0 || proxi >= seuil)) {
        ...
        vPopulation[2+currentElite].nbEdgesConflict=1e9;
...
void Head::printPopulation(ostream& st){
    ...
    st<<"\tprox="<<1.L * proxi / graph->nbSommets;
