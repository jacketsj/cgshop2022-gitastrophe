...
struct ColoringError {
	ColoringError(std::size_t s1, std::size_t s2, std::size_t cc, double x,
								double y, std::string m, bool ii) noexcept
...
public:
	 ...
	 */
	ColoringVerifier(const std::vector<Segment>& segs,
									 const std::vector<std::size_t>& coloring) noexcept
	...
	std::optional<ColoringError> verify() {
		...
		for (const auto& cclass : color_classes) {
			verify_color_class(cclass.first, cclass.second);
			if (m_intersection) {
				return m_intersection;
			}
		}
...
private:
	...
	bool sanity_check() {
		if (m_segments->size() != m_coloring->size()) {
			...
			msg << "Number of colored segments (" << m_coloring->size()
					...
					<< m_segments->size() << ")";
			m_intersection =
					ColoringError{npos, npos, npos, qnan, qnan, msg.str(), false};
			return false;
		}
		return true;
	}
	 ...
	 * an error is found.
	 */
	void verify_color_class(std::size_t color,
													const std::vector<std::size_t>& color_class) {
		...
		if (result) {
			std::size_t i1 = result->segment_index[0] =
					subindex_to_index[result->segment_index[0]];
			std::size_t i2 = result->segment_index[1] =
					subindex_to_index[result->segment_index[1]];
			Segment s1 = (*m_segments)[i1], s2 = (*m_segments)[i2];
			std::stringstream msg;
			msg << "Intersection between segments " << i1 << " (" << s1 << ") and "
					<< i2 << " (" << s2 << ") near (" << result->approximate_location[0]
					<< ", " << result->approximate_location[1] << ")";
			m_intersection = ColoringError{i1,
																		 i2,
																		 color,
																		 result->approximate_location[0],
																		 result->approximate_location[1],
																		 msg.str(),
																		 true};
		}
