...
struct Point {
	...
	Point(std::int64_t x, std::int64_t y) noexcept : x(x), y(y) {}
 ...
 */
struct Segment {
	Segment(Point s, Point t) noexcept : s(s), t(t) {}
...
static inline bool collinear_order_correct(Point p1, Point p2,
																					 Point p3) noexcept {
	if (p1.x == p2.x) {
...
static inline bool ranges_overlap(std::int64_t min1, std::int64_t max1,
																	std::int64_t min2, std::int64_t max2) {
	if (min1 < min2) {
		return max1 >= min2;
	} else {
		return max2 >= min1;
 ...
 */
inline std::array<double, 2> approximate_intersection_point(Segment s1,
																														Segment s2) {
	Point p = s1.s;
	...
	if (!qpr && !rs) { // collinear
		...
		throw std::runtime_error(
				"Intersection check positive, but no intersection!");
	} else {
...
/**
 * Do the given segments intersect? If yes, an approximate intersection point is
 * returned. Both s1 and s2 must have si.s <= si.t.
 */
inline std::optional<std::array<double, 2>> do_intersect(Segment s1,
																												 Segment s2) {
	...
	int o1 = orientation(s1.s, s1.t, s2.s), o2 = orientation(s1.s, s1.t, s2.t),
			o3 = orientation(s2.s, s2.t, s1.s), o4 = orientation(s2.s, s2.t, s1.t);
	if (o1 != o2 && o3 != o4) { // general case
		return approximate_intersection_point(s1, s2);
	}
	if (!o1 && !o2) { // all collinear
		std::int64_t mnxr1 = (std::min)(s1.s.x, s1.t.x),
								 mxxr1 = (std::max)(s1.s.x, s1.t.x);
		std::int64_t mnyr1 = (std::min)(s1.s.y, s1.t.y),
								 mxyr1 = (std::max)(s1.s.y, s1.t.y);
		std::int64_t mnxr2 = (std::min)(s2.s.x, s2.t.x),
								 mxxr2 = (std::max)(s2.s.x, s2.t.x);
		std::int64_t mnyr2 = (std::min)(s2.s.y, s2.t.y),
								 mxyr2 = (std::max)(s2.s.y, s2.t.y);
