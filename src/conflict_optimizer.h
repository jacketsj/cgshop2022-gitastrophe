...
struct conflict_improver {
  ...
  conflict_improver(string filename, int time_limit = 1e9): ins(filename), s(ins) {
    s.from_json_default();
    rng = mt19937(time(0));
    bad.resize(ins.m);
    qcnt.resize(ins.m);
  ...
  void init_bad_colour() {
    ...
    while (!bad_edges.empty()) bad_edges.pop_back();
  ...
  void run() {
    while (1) {
      ...
      while (!bad_edges.empty()) {
        if (iter % 1000000 == 0) {
          cerr << setw(18) << setfill(' ') << ins.id << " Running iteration " << iter << ": "
               << bad_edges.size() << " edges remaining.\n";
        }
        /*
        if (iter % 1000000 == 0) {
        ...
        if (iter++ % 10000 == 0 && std::chrono::steady_clock::now() >= limit) {
          ...
          //cerr << "Running iteration " << iter << ": " << bad_edges.size() << " remaining.\n";
				...
				/*
        if (iter >= switchiter - 1e4 && (bestq.empty() || bad_edges.size() <= bestq.size())) {
          bphase = s.cols;
          bestq  = bad_edges;
        }*/
				mx_qsz = max(mx_qsz, (int64_t)bad_edges.size());
        swap(bad_edges[rng()%bad_edges.size()], bad_edges.back());
        int e = bad_edges.back(); bad_edges.pop_back();
        if (mode) qcnt[e]++;
        ...
        if (mode) {
          ...
          int64_t mnscore = *min_element(begin(conflict_score), end(conflict_score));
          c = rng() % (s.num_cols - 1);
        ...
        if (iter == switchiter) {
          ...
          mode ^= 1;
          if (mode == 1) switchiter += 2e4;
          else switchiter += 1e4;
        }
      }
      cerr << "Successful improvement! " << s.num_cols << " -> ";
      s.recompute_num_cols();
      cerr << s.num_cols << " mx_qcnt=" << mx_qcnt << " mx_qsz=" << mx_qsz << " restarts=" << restarts << " num_iters=" << iter << endl;
      assert(s.verify());
...
struct conflict_improver_2 {
  ...
  vector<size_t> cols;
  std::chrono::time_point<std::chrono::steady_clock> limit;
  conflict_improver_2(solution& _s, int time_limit, mt19937& _rng): ins(_s.ins), s(_s), rng(_rng) {
    bad.resize(ins.m);
    qcnt.resize(ins.m);
    limit = std::chrono::steady_clock::now() + std::chrono::milliseconds(time_limit);
    cols = s.cols;
    init_bad_colour();
    run_sub();
  }
    ...
    while (1) {
      ...
      while (!bad_edges.empty()) {
        ...
        if (iter++ % 200 == 0 && std::chrono::steady_clock::now() >= limit) {
          s.cols = cols;
          //bad_edges = bestq;
          cerr << "returning after " << iter << " iters, " << bad_edges.size() << " remaining\n";
          /*
          int c = s.num_cols-1;
          vector<int> bades;
          while (!bad_edges.empty()) {
